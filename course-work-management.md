...
Stage 2 authority was extracted from the earlier production-history chain so that current learner requirements no longer have to be reconstructed from `course-design/production/cycle-1/`.
...
- `course-design/stage-1-3-target-topology-decision-2026-09-15.md` together with its review/post-build records — current Stage 1–3 spatial-role topology;
...
Historical production artifacts remain evidence/provenance unless a current authority source explicitly incorporates or points to them.
...
- Waves 1–4 are complete;
- Wave 5A narrow response-aware feedback is complete;
...
This management file does not restate the detailed Wave decisions. Use the initiative record and its referenced decision/audit artifacts for that evidence.

## 6. Current next action

The next action is **not implementation** and is not an automatic next Wave.

Perform a separate evidence-based **category-selection / mapping decision** across the remaining learner-experience improvement queue.

That decision should:

1. inspect the current Stage 1–3 learner experience and current authority;
2. identify the strongest remaining learner-experience problem supported by evidence;
3. classify it as `CONFORMANCE`, `IMPLEMENTATION CHOICE`, `CANON DECISION REQUIRED`, or `POTENTIAL CONFLICT`;
4. define the smallest justified mapping scope;
5. avoid runtime change until the mapping decision is complete.

The authoritative work-state detail for this initiative remains in `course-experience-improvement-work-management.md`.
...
## 9. Current non-blocking open areas

Open matters should enter active work only when evidence or implementation makes them necessary. Current examples include:
...
These are not automatically blockers for the current category-selection / mapping decision.
...
`learner-encounter-production-execution.md` is currently a **historical / superseded Cycle 1 execution snapshot**, not the project-wide current-action source.
