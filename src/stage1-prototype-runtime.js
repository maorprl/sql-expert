...
export function createStage1Prototype({ root, getDatabase, onContinue }) {
  ...
  function reset() {
    ...
    $('#s1-entry').hidden = false; $('#s1-ws').className = 'ws locked'; $('#s1-link').className = 'link'; $$('#s1-link .cd').forEach((marker) => { marker.hidden = true; });
    ...
    state = 'relations'; spine('Requested rows: articles + publishing-source names'); markCurrentAction('#s1-entry'); conversation.scrollTop = 0;
  }
  ...
  function afterGrain() {
    ...
    teacher('The requested Grain is one article per result row. Now establish a separate fact: the number of starting article rows to use as the comparison baseline. Run the prepared measurement below.'); $('#s1-measure').hidden = false; $('#s1-ws').classList.add('dim'); $('#s1-run-measure').focus();
  ...
  function afterSemantic() {
    concept('JOIN', 'A <strong>JOIN</strong> combines matching rows from related relations. It is the SQL operation for the matching logic you already used to predict this step’s row effect.'); state = 'join-teaching'; $('#s1-teaching').hidden = false;
    markCurrentAction('#s1-teaching');
    teacher('Follow the same argument into SQL: first one matching pair, then the <code>ON</code> condition that finds it, then the full query that applies that logic to the 18 starting article rows.'); $('#s1-teach-board').scrollIntoView({ block: 'nearest' });
  }
  ...
  function executeSql() {
    ...
    try {
      ...
      renderResults(columns, values); setExecutionStatus('Verified · result satisfies the task', 'verified'); state = 'verification'; $('#s1-execute').disabled = true; teacher('The query executed the matching logic and returned evidence. Compare those rows with the 18-row prediction and the established article Grain. What does the evidence establish?'); askVerification();
  ...
  function complete() {
    ...
    spine('Argument closed: one match per article → predicted 18 → verified 18'); $('#s1-enrich').hidden = false; $('#s1-enrich-panel').innerHTML = '<button class="enrichment-btn" id="s1-enrich-toggle">Go deeper: How the JOIN produced this result</button>';
