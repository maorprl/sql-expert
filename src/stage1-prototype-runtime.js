...
export function createStage1Prototype({ root, getDatabase, onContinue }) {
  ...
  function complete() {
    ...
    const completion = add(node('<div class="completion"><div class="completion-eyebrow">Success · verified result</div><h3>Lesson 1 complete</h3><p>The research team can now read every article with its publishing source. Each article matched one source, so adding source information kept the result at 18 rows and one article per row.</p><p class="completion-next">Continue within RouteCraft · SQL Lab to Funding participation.</p><button class="primary continue-stage" type="button">Continue to Funding participation</button></div>'));
  ...
  $('#s1-to-sql').addEventListener('click', () => { state = 'sql'; const activeTeaching = $('.teach-step.active'); activeTeaching.open = false; activeTeaching.classList.remove('active'); activeTeaching.classList.add('completed'); $('#s1-sql').hidden = false; markCurrentAction('#s1-sql'); renderInspector(); teacher('The SQL workspace is ready. Write the query that implements the relationship, keeps the established one-article Grain, and tests the predicted 18-row count.'); $('#s1-sql-editor').focus(); });
