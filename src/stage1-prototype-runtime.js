    ...
    <main class="app"><div class="zones">
      ...
      <section class="workbench" aria-label="Lesson workspace">
        ...
        <div class="wb-block" id="s1-teaching" hidden><div class="wb-head"><span class="eyebrow">JOIN teaching</span><h3>Follow the relationship you established</h3></div><div class="teach-board" id="s1-teach-board">
          ...
          <details class="teach-step" data-step="3" hidden><summary><span class="teach-kicker">Connect the full query</span><span class="teach-title">The query expresses the request</span></summary><div class="teach-content"><div class="teach-copy">The request asks for every article with the name of its publishing source. One requested result row represents one article. The complete query carries that meaning through every clause:</div><div class="query-map"><div><code>FROM news_article</code><span>starts from the article rows</span></div><div><code>JOIN news_source</code><span>brings in the matching source row</span></div><div><code>ON news_article.news_source_id = news_source.news_source_id</code><span>defines how the rows match</span></div><div><code>SELECT news_article.title, news_source.name</code><span>chooses the article and source fields that appear</span></div></div><p class="teach-conclusion">Because each article matches one source, the query as a whole keeps the established Grain: one article per result row.</p><div class="teach-actions"><button class="primary" id="s1-to-sql">Open the SQL workspace</button></div></div></details>
        </div></div>
        <div class="wb-block" id="s1-sql" aria-label="SQL workspace" hidden><div class="wb-head"><span class="eyebrow">SQL workspace</span><h3>Make the argument executable</h3></div><div class="editor"><div class="editor-gutter">1<br>2<br>3<br>4<br>5</div><textarea class="sql-editor" id="s1-sql-editor" spellcheck="false" aria-keyshortcuts="Control+Enter Meta+Enter" placeholder="Write a SELECT that joins news_article to news_source…"></textarea></div><div class="editor-actions" id="s1-sql-actions"><button class="primary" id="s1-execute">Execute SQL</button><button class="secondary" id="s1-assist-button">Show a nudge</button><button class="secondary" id="s1-solution-button">Show solution</button><span class="run-note">Runs against the course database and checks the returned meaning.</span></div><div class="execution-status" id="s1-execution-status" role="status" aria-live="polite" hidden></div><div id="s1-assist" hidden></div><div id="s1-diagnostic" role="status" aria-live="polite"></div><div id="s1-result"></div></div>
...
export function createStage1Prototype({ root, getDatabase, onContinue }) {
  ...
  function beginConnection() {
    ...
    teacher('These are the two places where the requested information lives: the article relation and the publisher relation. Now point to the column in an article row that <strong>identifies the source that published it</strong>.');
  ...
  function onColumn(button) {
    ...
    concept('Primary key / foreign key', '<code>news_article.news_source_id</code> is a required <strong>foreign key</strong>. It stores one value that references <code>news_source.news_source_id</code>, the source relation’s <strong>primary key</strong>. The link tells us which fields connect; we still need to reason about how many rows can participate in each direction.');
    teacher('Start from one article row and use the required foreign key and the publishing meaning of the data.');
    ask({ prompt: 'For one article, how many publishing sources can this relationship identify?', options: [['one', 'Exactly one publishing source.'], ['many', 'Many publishing sources.'], ['none', 'No publishing source.']], correct: 'one', wrong: { many: 'The article row stores one required <code>news_source_id</code>, and that value identifies one source primary key.', none: 'The article’s <code>news_source_id</code> is required and identifies a source row.' }, onCorrect: afterArticleDirection });
  }

  function afterArticleDirection() {
    spine('Article direction: one article → one source');
    state = 'cardinality-source-direction';
    teacher('Now reason in the other direction. <code>news_article.news_source_id</code> is not constrained <code>UNIQUE</code>. Consider what that structure allows in the publishing domain.');
  ...
  function afterSemantic() {
    ...
    teacher('Follow three connected explanations: first the row match, then the <code>ON</code> match condition, then the full query mapped to the request.'); $('#s1-teach-board').scrollIntoView({ block: 'nearest' });
  }
  ...
  function complete() {
    ...
    concept('JOIN verified', 'The relationship reasoning established the Grain and predicted the row count before the query existed; the actual result confirmed consistency with both. SQL is not the conclusion by itself — the verified meaning of its result is.');
    $('#s1-sql-editor').disabled = true; $('#s1-sql-actions').hidden = true; $('#s1-next').disabled = false;
    const completion = add(node('<div class="completion"><div class="completion-eyebrow">Success · verified result</div><h3>Lesson 1 complete</h3><p>You carried one argument from request to evidence: relations, link, cardinality, grain, baseline, prediction, JOIN, <code>ON</code>, SQL, and a verified 18-row result. Source attributes were added while the result remained one row per article.</p><p class="completion-next">Continue within RouteCraft · SQL Lab to Funding participation.</p><button class="primary continue-stage" type="button">Continue to Funding participation</button></div>'));
    completion.querySelector('.continue-stage').addEventListener('click', () => onContinue?.());
    spine('Lesson 1: argument closed'); $('#s1-enrich').hidden = false; $('#s1-enrich-panel').innerHTML = '<button class="enrichment-btn" id="s1-enrich-toggle">Go deeper: How the JOIN produced this result</button>';
  ...
  $('#s1-to-sql').addEventListener('click', () => { state = 'sql'; $('#s1-sql').hidden = false; $('#s1-teaching').classList.remove('is-current-action'); markCurrentAction('#s1-sql'); renderInspector(); teacher('The SQL workspace is ready. Write the query that implements the relationship, keeps the established one-article Grain, and tests the predicted 18-row count.'); $('#s1-sql-editor').focus(); });
  ...
  $('#s1-solution-button').addEventListener('click', () => { $('#s1-sql-editor').value = SOLUTION; setExecutionStatus('Not executed', 'idle'); $('#s1-sql-editor').focus(); $('#s1-diagnostic').innerHTML = '<div class="assist">The solution is now in the editor. It has not run, and the lesson has not advanced.</div>'; });
