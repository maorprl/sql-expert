...
const RELATION_INFO = {
  funding_round: { description: 'Recorded funding rounds and their announced context.', columns: [['funding_round_id', 'INTEGER', 'The funding-round record identifier.'], ['round_type', 'TEXT', 'The category of funding round.'], ['announced_date', 'TEXT', 'The date the round was announced.']] },
  round_investment: { description: 'Recorded investor participation in funding rounds.', columns: [['round_investment_id', 'INTEGER', 'The participation record identifier.'], ['funding_round_id', 'INTEGER', 'The recorded funding_round_id value.'], ['investor_id', 'INTEGER', 'The recorded investor identifier.'], ['is_lead', 'INTEGER', 'Whether this investor was recorded as lead.']] },
  ...
  news_article: { description: 'Articles published by news sources.', columns: [['news_article_id', 'INTEGER', 'The article record identifier.'], ['title', 'TEXT', 'The published article title.']] },
};

const template = `
<div class="stage2-routecraft">
  <header class="masthead"><div><div class="mh-course">RouteCraft · SQL Lab</div><div class="mh-title">Funding participation — who invested in which round</div></div><div class="mh-right"><span class="mh-continuation" id="s2-continuation" hidden>Continued from Stage 1</span><span class="mh-stage">Stage 2 · participation grain</span><nav class="lesson-nav" aria-label="Course lessons"><button class="lesson-nav-button" id="s2-previous" type="button">Previous</button><button class="lesson-nav-button" id="s2-next" type="button" disabled>Next</button></nav><button class="ghost" id="s2-restart">Restart</button></div></header>
  <main class="app"><div class="zones">
    ...
    <section class="workbench" aria-label="Workbench">
      <div class="wb-block" id="s2-entry"><div class="wb-head"><span class="eyebrow">Schema catalog</span><h3>Bring in the relations this request needs</h3></div><div class="catalog" id="s2-catalog">
        <div class="catalog-card" data-rel="funding_round"><button class="catalog-select" type="button"><span><span class="catalog-name">funding_round</span><span class="catalog-desc">round type and announced date</span></span><span class="catalog-status">add</span></button><button class="inspect-relation" type="button">Inspect</button></div>
        <div class="catalog-card" data-rel="round_investment"><button class="catalog-select" type="button"><span><span class="catalog-name">round_investment</span><span class="catalog-desc">recorded investor participations</span></span><span class="catalog-status">add</span></button><button class="inspect-relation" type="button">Inspect</button></div>
      ...
      </div><div class="catalog-feedback" id="s2-catalog-feedback" role="alert" hidden></div><div class="wb-head bench-head"><span class="eyebrow">Working schema</span><h3>Relations on the bench</h3></div><div class="hint" id="s2-bench-hint">Pick the two relations that hold the participation and the round it belongs to.</div></div>
      <div class="wb-block" id="s2-schema" hidden><div class="wb-head"><span class="eyebrow">Working schema</span><h3>Two relations, one link</h3></div><div class="wws" id="s2-wws"><article class="card" data-rel="funding_round"><div class="card-head"><span>funding_round</span><button class="inspect-relation" type="button">Inspect</button></div><ul class="cols"><li><button class="col relationship-endpoint endpoint-left" data-rel="funding_round" data-col="funding_round_id"><code>funding_round_id</code><span class="badge" hidden>PK</span></button></li><li><button class="col" data-rel="funding_round" data-col="round_type"><code>round_type</code></button></li><li><button class="col" data-rel="funding_round" data-col="announced_date"><code>announced_date</code></button></li></ul></article><div class="link" id="s2-link"><span class="line"></span><span class="cd m">many</span><span class="cd one">one</span></div><article class="card" data-rel="round_investment"><div class="card-head"><span>round_investment</span><button class="inspect-relation" type="button">Inspect</button></div><ul class="cols"><li><button class="col" data-rel="round_investment" data-col="round_investment_id"><code>round_investment_id</code><span class="badge" hidden>PK</span></button></li><li><button class="col relationship-endpoint endpoint-right" data-rel="round_investment" data-col="funding_round_id"><code>funding_round_id</code><span class="badge badge-fk" hidden>FK</span></button></li><li><button class="col" data-rel="round_investment" data-col="investor_id"><code>investor_id</code></button></li><li><button class="col" data-rel="round_investment" data-col="is_lead"><code>is_lead</code></button></li></ul></article></div></div>
      <div class="wb-block" id="s2-measure" hidden><div class="wb-head"><span class="eyebrow">Baseline — reused</span><h3>Measure the participation side</h3></div><div class="editor"><div class="editor-gutter">1</div><pre class="editor-code">SELECT COUNT(*) FROM round_investment;</pre></div><div class="editor-actions"><button class="primary" id="s2-run-measure">Run measurement</button><span class="run-note" id="s2-run-note">Prepared again — you still don’t write this one.</span></div><div id="s2-measure-out" role="status" aria-live="polite"></div></div>
      ...
      <div class="wb-block" id="s2-sql" hidden><div class="wb-head"><span class="eyebrow">SQL authoring</span><h3>Author the query yourself</h3></div><div class="editor"><div class="editor-gutter">1<br>2<br>3<br>4<br>5<br>6</div><textarea class="sql-editor" id="s2-editor" spellcheck="false" aria-keyshortcuts="Control+Enter Meta+Enter" placeholder="Write a SELECT that joins round_investment to funding_round…"></textarea></div><div class="editor-actions" id="s2-actions"><button class="primary" id="s2-execute">Execute SQL</button><button class="secondary" id="s2-assist-button">Show a nudge</button><button class="secondary" id="s2-solution-button">Show solution</button><span class="run-note">Runs against the course database and checks the returned meaning.</span></div><div class="execution-status" id="s2-execution-status" role="status" aria-live="polite" hidden></div><div id="s2-assist" hidden></div><div id="s2-diagnostic" role="status" aria-live="polite"></div><div id="s2-result"></div><div id="s2-verify-evidence" hidden><div class="wb-head verify-head"><span class="eyebrow">Verification evidence</span><h3>Slice from the accepted result</h3></div><div class="slice" id="s2-verify-slice"></div></div></div>
      ...
      <div class="wb-block" id="s2-completion" hidden><div class="completion"><div class="completion-eyebrow">Success · verified result</div><h3>Stage 2 complete</h3><p>You preserved one participation per row across 72 results and verified that repeated round context comes from one round matching multiple participation records.</p><p class="completion-distinction"><strong>Multiple matches:</strong> one round matched four participation rows, so JOIN carried the same round context into four distinct result rows without changing the Grain.</p><p class="completion-next">You have completed the currently available RouteCraft · SQL Lab journey.</p></div></div>
      <aside class="relation-inspector" id="s2-inspector" aria-labelledby="s2-inspector-title" hidden><div class="inspector-head"><div><span class="eyebrow">Relation inspector</span><h3 id="s2-inspector-title"></h3></div><button class="inspector-close" type="button" aria-label="Close relation inspector">×</button></div><div id="s2-inspector-body"></div><button class="secondary inspector-insert" id="s2-inspector-insert" type="button" hidden>Insert into SQL</button></aside>
    </section>
  </div></main>
</div>`;

export function createStage2Prototype({ root, getDatabase, onPrevious }) {
  ...
  const selected = new Set(); let state = 'request'; let acceptedResult = null; let inspectedRelation = null; let keyInfoRevealed = false; let relationshipInfoRevealed = false;
  ...
  const renderInspector = () => {
    ...
    const info = RELATION_INFO[inspectedRelation]; $('#s2-inspector-title').textContent = inspectedRelation;
    const keyLabels = keyInfoRevealed ? { funding_round_id: inspectedRelation === 'round_investment' ? 'FK' : 'PK', round_investment_id: 'PK' } : {};
    $('#s2-inspector-body').innerHTML = `<p class="inspector-description">${info.description}</p><ul class="inspector-fields">${info.columns.map(([name, type, description]) => `<li><div><code>${name}</code><span class="field-type">${type}</span>${keyLabels[name] ? `<span class="badge${keyLabels[name] === 'FK' ? ' badge-fk' : ''}">${keyLabels[name]}</span>` : ''}</div><p>${description}</p></li>`).join('')}</ul>${relationshipInfoRevealed && ['funding_round','round_investment'].includes(inspectedRelation) ? '<p class="inspector-relationship"><strong>Revealed relationship:</strong> many participations can name one funding round.</p>' : ''}`;
    $('#s2-inspector-insert').hidden = state !== 'sql';
  };
  const openInspector = (relation) => { inspectedRelation = relation; renderInspector(); $('#s2-inspector').hidden = false; };
  const closeInspector = () => { $('#s2-inspector').hidden = true; inspectedRelation = null; };
  const insertInspectedRelation = () => { if (state !== 'sql' || !inspectedRelation) return; const editor = $('#s2-editor'); editor.setRangeText(inspectedRelation, editor.selectionStart, editor.selectionEnd, 'end'); editor.focus(); };
  const setExecutionStatus = (message = '', status = '') => { const element = $('#s2-execution-status'); element.textContent = message; element.dataset.status = status; element.hidden = !message; };
  ...
  function reset() {
    state = 'request'; acceptedResult = null; selected.clear(); stream.innerHTML = ''; spineList.innerHTML = ''; keyInfoRevealed = false; relationshipInfoRevealed = false; closeInspector();
    ...
    $('#s2-editor').value = ''; $('#s2-editor').disabled = false; $('#s2-execute').disabled = false; $('#s2-actions').hidden = false; setExecutionStatus(); $('#s2-assist').hidden = true; $('#s2-assist').innerHTML = ''; $('#s2-diagnostic').innerHTML = ''; $('#s2-result').innerHTML = ''; $('#s2-verify-slice').innerHTML = '';
    ...
    spine('Request: every participation + its funding round'); state = 'relations'; markCurrentAction('#s2-entry'); conversation.scrollTop = 0;
  }
  function beginConnection() {
    if (state !== 'relations') return; state = 'connection'; $('#s2-entry').hidden = true; $('#s2-schema').hidden = false;
    spine('Relations: <code>funding_round</code> + <code>round_investment</code>'); markCurrentAction('#s2-schema'); teacher('Right. Which column in the participation relation carries the identity of the round it belongs to?');
  }
  function onColumn(button) {
    if (state !== 'connection') return; let feedback = $('#s2-connection-feedback'); if (!feedback) { feedback = make('<div class="ask-error" id="s2-connection-feedback" role="alert" hidden></div>'); add(feedback); }
  ...
  function showMechanism() {
    spine('Row behavior: repeated round context, Grain unchanged'); state = 'mechanism'; const evidence = source1003(); $('#s2-multi').hidden = false; markCurrentAction('#s2-multi');
    ...
    state = 'sql'; $('#s2-sql').hidden = false; $('#s2-required').hidden = false; markCurrentAction('#s2-sql'); renderInspector(); teacher('Your editor now. Six columns, the INNER JOIN, and the condition you established. I’ll be quiet while you write it.'); $('#s2-editor').focus();
  }
  function executeSql() {
    const sql = $('#s2-editor').value.trim(); const diagnostic = $('#s2-diagnostic'); diagnostic.innerHTML = ''; setExecutionStatus();
    ...
    try {
      ...
      acceptedResult = { columns: [...result.columns], values: result.values.map((row) => [...row]), indexes }; renderAccepted(); setExecutionStatus('Verified · result satisfies the task', 'verified'); state = 'verification'; $('#s2-execute').disabled = true; teacher('That is your accepted result, still on the bench. Compare its derived 1003 slice with your prediction and tell me what you are looking at.'); verify();
  ...
  function complete() {
    state = 'complete'; markCurrentAction(); renderInspector(); spine('Verified: <b>72</b> rows · round 1003 → 4 participations'); teacher('Exactly. The count stayed at 72, one per participation. Round 1003 appears in four distinct participation rows because one round matched four recorded participations — multiple matches, not duplicates.'); $('#s2-editor').disabled = true; $('#s2-actions').hidden = true; $('#s2-required').hidden = true; $('#s2-completion').hidden = false; spine('Stage 2: argument closed');
  }

  $('#s2-catalog').addEventListener('click', (event) => { const inspect = event.target.closest('.inspect-relation'); const card = event.target.closest('.catalog-card'); if (inspect && card) { openInspector(card.dataset.rel); return; } if (!event.target.closest('.catalog-select') || !card || state !== 'relations') return; const relation = card.dataset.rel; if (!['funding_round','round_investment'].includes(relation)) { $('#s2-catalog-feedback').hidden = false; $('#s2-catalog-feedback').textContent = 'You need recorded participations and the rounds they belong to.'; return; } $('#s2-catalog-feedback').hidden = true; if (selected.has(relation)) { selected.delete(relation); card.classList.remove('selected'); card.querySelector('.catalog-status').textContent = 'add'; } else { selected.add(relation); card.classList.add('selected'); card.querySelector('.catalog-status').textContent = 'on bench'; } if (selected.size === 2) setTimeout(beginConnection, 180); });
  ...
  $('#s2-solution-button').addEventListener('click', () => { $('#s2-editor').value = SOLUTION; setExecutionStatus('Not executed', 'idle'); $('#s2-editor').focus(); $('#s2-diagnostic').innerHTML = '<div class="assist">The solution is in the editor. It has not run, and the Stage has not advanced.</div>'; });
  ...
  $('#s2-previous').addEventListener('click', () => onPrevious?.());
  $('#s2-editor').addEventListener('keydown', (event) => { if (!event.isComposing && event.key === 'Enter' && (event.ctrlKey || event.metaKey)) { event.preventDefault(); executeSql(); } });
  $('#s2-editor').addEventListener('input', () => { if (state === 'sql') setExecutionStatus('Not executed', 'idle'); });
