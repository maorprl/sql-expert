...
export function createStage2Prototype({ root, getDatabase }) {
  ...
  function onColumn(button) {
    ...
    button.classList.add('picked'); feedback.hidden = true; $('#s2-link').classList.add('on'); $$('.badge').forEach((badge) => { badge.hidden = false; }); learner('<code>round_investment.funding_round_id</code>'); spine('Link: <code>round_investment.funding_round_id → funding_round.funding_round_id</code>'); state = 'premises'; markCurrentAction(); teacher('That’s the link. You already know keys and cardinality, so no new lesson — just confirm the premises you are carrying into this stage.'); premises();
  }
  function afterPremises() {
    $('#s2-link').classList.add('cardinality'); spine('Relationship: many participations → one round'); spine('Grain: one recorded participation per row'); state = 'baseline'; $('#s2-wws').classList.add('dim'); $('#s2-measure').hidden = false; markCurrentAction('#s2-measure'); teacher('Good. Measure the participation side and keep the number.'); $('#s2-run-measure').focus();
  }
  ...
  function showMechanism() {
    ...
    state = 'sql'; $('#s2-sql').hidden = false; $('#s2-required').hidden = false; markCurrentAction('#s2-sql'); teacher('Your editor now. Six columns, the INNER JOIN, and the condition you established. I’ll be quiet while you write it.'); $('#s2-editor').focus();
  }
  function executeSql() {
