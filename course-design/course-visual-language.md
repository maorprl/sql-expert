...
The control layer should remain easy to find and consistent across Stages where a control is available, while staying visually subordinate to the learner's current task, evidence, or authoring surface.

Back, Retry / Redo, and Show solution should therefore read as persistent course actions rather than as content generated by the current Stage. Exact placement, grouping, responsive treatment, and visual styling remain open implementation decisions.
