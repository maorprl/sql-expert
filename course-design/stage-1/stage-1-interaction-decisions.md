...
The baseline uses the persistent editor in compact mode and a visible Run Query button. The SQL task expands that editor, teaches the INNER JOIN pattern first, hides fields behind a non-hint control, and validates result semantics rather than exact SQL text.
