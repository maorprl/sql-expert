...
3. Identify `funding_round.company_id`, then introduce its relationship to the `company.company_id` primary key. Its value identifies the company row this funding round belongs to, and that row contains `status`.
